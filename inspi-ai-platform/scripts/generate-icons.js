...
function generateIcon(size, filename) {
  ...
  const svg = `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="${size * 0.35}" y="${size * 0.55}" width="${size * 0.3}" height="${size * 0.05}" rx="${size * 0.025}" fill="white"/>
...
console.log('Note: Generated as SVG files. For production, consider using a proper image conversion tool.');