...
export type GraphRecommendationType = 'missing_connection' | 'new_node' | 'work_suggestion';

export interface GraphRecommendation {
  type: GraphRecommendationType;
  description: string;
  confidence: number;
...
export interface NodeWorkStats {
  nodeId: string;
  workCount: number;
...
export interface GraphOperation {
  ...
  graphId: string;
  userId: string;
  type: 'create_node' | 'update_node' | 'delete_node' | 'create_edge' | 'delete_edge' | 'move_node' | 'mount_work' | 'unmount_work';
  data: Record<string, any>;
  timestamp: Date;
  canUndo: boolean;
}
