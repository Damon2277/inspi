...
import { FilterOptions } from '@/shared/types/square';
...
const FilterBar: React.FC<FilterBarProps> = ({
  ...
  const handleSubjectClick = (subject: string) => {
    if (selectedSubject === subject) {
      onSubjectChange && onSubjectChange(undefined);
    } else {
      onSubjectChange && onSubjectChange(subject);
    }
  };

  const handleGradeLevelClick = (gradeLevel: string) => {
    if (selectedGradeLevel === gradeLevel) {
      onGradeLevelChange && onGradeLevelChange(undefined);
    } else {
      onGradeLevelChange && onGradeLevelChange(gradeLevel);
    }
  };
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {selectedSubject && (
              <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-indigo-100 text-indigo-800">
                学科: {selectedSubject}
                <button
                  onClick={() => onSubjectChange && onSubjectChange(undefined)}
            ...
            {selectedGradeLevel && (
              <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-indigo-100 text-indigo-800">
                ...
                <button
                  onClick={() => onGradeLevelChange && onGradeLevelChange(undefined)}
                  className="ml-2 inline-flex items-center justify-center w-4 h-4 text-indigo-600 hover:text-indigo-800"
                >
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
