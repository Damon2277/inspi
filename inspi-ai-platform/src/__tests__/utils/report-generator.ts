...
export class ReportGenerator {
  ...
  private async performQualityGateCheck(report: TestReport): Promise<void> {
    ...
    // 覆盖率检查
    const coverageThreshold = parseInt(process.env.COVERAGE_THRESHOLD || '90')
    rules.push({
      name: 'coverage',
    ...
    // 性能检查
    const performanceThreshold = parseInt(process.env.PERFORMANCE_THRESHOLD || '3000')
    rules.push({
      name: 'performance',
    ...
    // 安全检查
    const securityThreshold = parseInt(process.env.SECURITY_THRESHOLD || '0')
