  ...
  async getOptimizedInviteCode(code: string): Promise<any> {
    try {
      ...
      if (this.databaseOptimizer) {
        ...
        const result = await this.databaseOptimizer.executeOptimizedQuery(optimizedQuery)
        
        // 缓存结果
        if (this.cacheManager && result.length > 0) {
          await this.cacheManager.cacheInviteCode(code, result[0])
        }
  ...
  async getOptimizedUserStats(userId: string): Promise<any> {
    try {
      ...
      if (this.databaseOptimizer) {
        ...
        // 缓存结果
        if (this.cacheManager && result.length > 0) {
          await this.cacheManager.cacheUserStats(userId, result[0])
        }
