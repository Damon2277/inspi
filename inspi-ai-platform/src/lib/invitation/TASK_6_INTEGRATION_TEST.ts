...
export async function testNotificationSystemIntegration(): Promise<boolean> {
  try {
    ...
    // 创建通知服务实例
    const notificationService = new NotificationServiceImpl(mockDb)
    const eventHandler = new NotificationEventHandler(notificationService)
    const scheduler = new NotificationScheduler(notificationService, eventHandler)
    ...
    await eventHandler.handleInviteSuccess('user1', 'user2', '张三')
