...
export class StatisticsUpdateServiceImpl implements StatisticsUpdateService {
  ...
  async recalculateAllStatistics(): Promise<void> {
    try {
      ...
      const users = await this.db.query(usersQuery)
  ...
  async getUpdateStatus(): Promise<{
    ...
    try {
      ...
      const result = await this.db.queryOne(lastUpdateQuery)
  ...
  async validateStatisticsConsistency(): Promise<{
    ...
    try {
      ...
      results.forEach((row: any) => {
        if (row.stats_total_invites !== row.actual_total_invites) {
          inconsistencies.push({
            userId: row.user_id,
            field: 'total_invites',
            expected: row.actual_total_invites,
            actual: row.stats_total_invites
          })
        }

        if (row.stats_registrations !== row.actual_registrations) {
          inconsistencies.push({
            userId: row.user_id,
            field: 'successful_registrations',
            expected: row.actual_registrations,
            actual: row.stats_registrations
          })
        }

        if (row.stats_active !== row.actual_active) {
          inconsistencies.push({
            userId: row.user_id,
            field: 'active_invitees',
            expected: row.actual_active,
            actual: row.stats_active
          })
        }
      })
