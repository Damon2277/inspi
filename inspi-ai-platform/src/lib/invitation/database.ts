...
export class DatabaseService {
  private poolPromise: Promise<DatabasePool>;

  constructor(pool?: DatabasePool, config?: DatabaseConfig) {
    if (pool) {
      this.poolPromise = Promise.resolve(pool);
    } else {
      this.poolPromise = this.resolvePool(config);
    }
  }

  private async resolvePool(config?: DatabaseConfig): Promise<DatabasePool> {
    try {
      return DatabaseFactory.getInstance();
    } catch (error) {
      const finalConfig = config || getDatabaseConfig();
      return DatabaseFactory.createPool(finalConfig);
    }
  }

  private async getPool(): Promise<DatabasePool> {
  ...
  async transaction<T>(handler: (connection: DatabaseConnection) => Promise<T>): Promise<T> {
    const pool = await this.getPool();
