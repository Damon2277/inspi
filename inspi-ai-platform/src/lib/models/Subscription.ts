...
type SubscriptionPaymentMethod = PaymentMethod | 'alipay';
...
export interface SubscriptionDocument extends SubscriptionAttributes, Document {
  isActive(): boolean;
  isExpired(): boolean;
...
const SubscriptionSchema = new Schema<SubscriptionDocument>({
  ...
  plan: {
    type: String,
    enum: ['free', 'basic', 'pro', 'admin'] as UserTier[],
    required: true,
    default: 'free',
  },
  status: {
    type: String,
    enum: ['active', 'cancelled', 'expired', 'pending', 'suspended', 'inactive'] as SubscriptionStatus[],
    required: true,
    default: 'active',
  },
  startDate: {
  ...
  paymentMethod: {
    type: String,
    enum: ['wechat_pay', 'alipay'] as SubscriptionPaymentMethod[],
    required: true,
    default: 'wechat_pay',
  },
}, {
...
SubscriptionSchema.methods.getDaysRemaining = function (this: SubscriptionDocument): number {
  const now = new Date();
...
SubscriptionSchema.pre('findOneAndUpdate', function (next) {
  const update = this.getUpdate() as Partial<SubscriptionAttributes> & { autoRenew?: boolean };
